...
public class RotateAction : StateAction
{
    ...
    private       RotateActionSO _originSO => (RotateActionSO)OriginSO; // The SO this StateAction spawned from
    ...
    {
        var horizontalMovement = _protagonistScript.movementVector;
        horizontalMovement.y = 0f;

        if (horizontalMovement.sqrMagnitude >= ROTATION_TRESHOLD)
        {
            var targetRotation = Mathf.Atan2(_protagonistScript.movementVector.x, _protagonistScript.movementVector.z) * Mathf.Rad2Deg;
