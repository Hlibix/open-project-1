...
public class ReplaceTool : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        var objectToReplaceCount = data.objectsToReplace != null ? data.objectsToReplace.Length : 0;
    ...
    {
        InitDataIfNeeded();
        var objectFilter = SelectionMode.Unfiltered ^ ~(SelectionMode.Assets | SelectionMode.DeepAssets | SelectionMode.Deep);
    ...
    internal static void ReplaceSelectedObjects(GameObject[] objectToReplace, GameObject replaceObject)
    {
        ...
        for (var i = 0; i < objectToReplace.Length; i++)
        {
