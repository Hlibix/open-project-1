...
{
    [SerializeField]
    private Image _imagePaginationPrefab;

    [SerializeField]
    private Transform _parentPagination;

    [SerializeField]
    private Sprite _emptyPagination;

    [SerializeField]
    private Sprite _filledPagination;

    private HorizontalLayoutGroup horizontalLayout;

    private List<Image> _instantiatedImages;
    ...
    public void SetPagination(int paginationCount, int selectedPaginationIndex)
    {
        ...
        var maxCount = Mathf.Max(paginationCount, _instantiatedImages.Count);
        if (maxCount > 0)
        {
            for (var i = 0; i < maxCount; i++)
            {
                ...
                {
                    var instantiatedImage = Instantiate(_imagePaginationPrefab, _parentPagination);
    ...
    public void SetCurrentPagination(int selectedPaginationIndex)
    {
        ...
        {
            for (var i = 0; i < _instantiatedImages.Count; i++)
            {
