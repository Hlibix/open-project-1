...
{
    [SerializeField]
    private UISettingsLanguageComponent _languageComponent;

    [SerializeField]
    private UISettingsGraphicsComponent _graphicsComponent;

    [SerializeField]
    private UISettingsAudioComponent _audioComponent;

    [SerializeField]
    private UISettingTabsFiller _settingTabFiller;

    [SerializeField]
    private SettingsSO _currentSettings;

    [SerializeField]
    private List<SettingsType> _settingTabsList = new();
    ...
    private void SwitchTab(float orientation)
    {
        if (orientation != 0)
        {
            ...
            var initialIndex = _settingTabsList.FindIndex(o => o == _selectedTab);
            if (initialIndex != -1)
            {
                if (isLeft)
                {
